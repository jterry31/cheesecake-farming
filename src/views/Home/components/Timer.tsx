...
const TimerBox = styled.div<{ isMobile: boolean}>`
  display: ${({ isMobile }) => (isMobile ? "grid" : `flex`)};
  grid-template-columns: ${({ isMobile }) => (isMobile ? "1fr 1fr" : `null`)};
...
const Timer: React.FC = () => {
  ...
  const endTime = 1626181200; // use UNIX timestamp in seconds
