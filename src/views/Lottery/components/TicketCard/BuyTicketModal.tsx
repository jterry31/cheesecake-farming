...
const BuyTicketModal: React.FC<BuyTicketModalProps> = ({ max, onDismiss }) => {
  ...
  return (
    <Modal title={TranslateString(450, 'Enter amount of tickets to buy')} onDismiss={onDismiss}>
      ...
      />
      <div>
          <Tips>{`Your amount must be a multiple of ${ticketPrice} CCAKE`}</Tips>
          <Tips>{`1 Ticket = ${ticketPrice} CCAKE`}</Tips>
      </div>
      <div>
        <Announce>
          {TranslateString(
        ...
        <Final>{TranslateString(460, `You will spend: ${cakeCosts(val)} CCAKE`)}</Final>
      </div>
      <ModalActions>
