...
const StyledNav = styled.nav<{ showMenu: boolean }>`
  ...
  background-color: rgba(99, 54, 67, 0.8);
      ...
      <StyledNav showMenu={showMenu}>
          ...
          {links.map((link, index) => {
              ...
              return (
                <Link key={link.label} href={link.href} aria-label={link.label} mr={mr1}>
                  ...
                  <Text ml="8px" mr="8px" color="textSubtle"> {link.label} </Text>
                </Link>
              );
            })}
          </LinkEntry>}
        <Flex>
          ...
          {socials.map((social, index) => {
              ...
              const mr = index < socials.length - 1 ? "16px" : "16px";
              if (social.items) {
                return (
              ...
              return (
                <Link external key={social.label} href="a" aria-label={social.label} mr={mr}>
                  <Icon {...iconProps} />
                </Link>
              );
            })}
          ...
          { cakePriceUsd ? (
            <PriceLink href={priceLink} target="_blank">
              {!isMobile && <PancakeRoundIcon width="32px" mr="8px" />}
              <Text color="textSubtle" mr="20px" fontSize="18px" bold>{`$${cakePriceUsd.toFixed(3)}`}</Text>
            </PriceLink>
          ) : (!isMobile && 
